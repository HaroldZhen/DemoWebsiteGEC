...
$geClass = new cyut();    // NewClass cyut only for thie cyut ge website
  ...
  //GatData: m_meun
  $query_str = "select * from m_meun where b_meun_id=5 and close=1 order by m_meun_id,m_sort asc";
  ...
  //GetData: epaper
  if(isset($_GET['num']) ){
    $getNum = isset($_GET['num'])? $_GET['num'] : '19';
    $query_str = "select * from epaper where m_meun_id='".$getNum."'";
  }else{
    $query_str = "select * from epaper order by epaper_id asc";
  }
  $epaper_ary = $geClass->mdb2GetRow($query_str);
  $smarty->assign('text',$epaper_ary);
